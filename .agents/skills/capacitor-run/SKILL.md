...
argument-hint: [android] (por defecto: android — iOS no ejecutable desde Windows)
---
...
4. **Síntoma específico de Capacitor**: WebView en blanco con proceso vivo y sin `FATAL EXCEPTION` = build web no llegó al nativo (sync olvidado) o `webDir` mal configurado. Ver tabla de síntomas en `docs/CAPACITOR.md` antes de asumir un bug de código.

### iOS: fuera de alcance en esta máquina

No hay simulador ni `xcodebuild` en Windows. Cualquier pedido de "ejecuta en iOS" se responde derivando a la sección CI de `docs/CAPACITOR.md` — no hay equivalente local que probar.
